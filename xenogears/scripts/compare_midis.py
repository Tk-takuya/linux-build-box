...
def main():
    ...
    if not generated.exists():
        raise SystemExit(f'generated MIDI not found: {generated}')
    fixed_fft = Path(sys.argv[2])
    fixed_pc0 = Path(sys.argv[3])
    dump('GENERATED', summarize(generated))
    dump('FIXED_FFT118', summarize(fixed_fft))
    dump('FIXED_PC0', summarize(fixed_pc0))
